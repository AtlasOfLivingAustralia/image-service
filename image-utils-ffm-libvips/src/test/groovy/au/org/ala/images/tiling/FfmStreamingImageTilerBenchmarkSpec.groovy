...
class FfmStreamingImageTilerBenchmarkSpec extends Specification {
    ...
    def "benchmark FFM tiler variants on medium image"() {
        given:
        if (IMAGE_MODE == "large") {
            println "[BENCHMARK] Skipping FFM medium benchmark due to tiler.bench.image=${IMAGE_MODE}"
            return
        }
        if (!NativeLibraryDetectorFFM.isVipsAvailable()) {
            println "[BENCHMARK] Skipping FFM benchmark - libvips not available"
            return
        }
        if (!mediumImage?.exists()) {
            println "[BENCHMARK] Skipping FFM medium benchmark - image not found: ${mediumImage}"
            return
        }
        ...
        // FfmStreamingImageTiler uses levelExecutor (second param), not ioExecutor
        def sequential = new FfmStreamingImageTiler(null, new ImageTilerConfig(null, sameThread, TILE_SIZE, 6, TileFormat.JPEG, null, VIPS_CONCURRENCY))
        def fixedPoolTiler = new FfmStreamingImageTiler(null, new ImageTilerConfig(null, fixedPool, TILE_SIZE, 6, TileFormat.JPEG, null, VIPS_CONCURRENCY))
        def virtualThreadTiler = new FfmStreamingImageTiler(null, new ImageTilerConfig(null, virtualPool, TILE_SIZE, 6, TileFormat.JPEG, null, VIPS_CONCURRENCY))
    ...
    def "benchmark FFM tiler variants on large image"() {
        given:
        if (IMAGE_MODE == "medium") {
            println "[BENCHMARK] Skipping FFM large benchmark due to tiler.bench.image=${IMAGE_MODE}"
            return
        }
        if (!NativeLibraryDetectorFFM.isVipsAvailable()) {
            println "[BENCHMARK] Skipping FFM benchmark - libvips not available"
        ...
        if (!largeImage?.exists()) {
            println "[BENCHMARK] Skipping FFM large benchmark - image not found: ${largeImage}"
            return
        }

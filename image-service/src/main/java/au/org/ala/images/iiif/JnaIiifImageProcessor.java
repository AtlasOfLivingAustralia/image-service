 ...
 */
public class JnaIiifImageProcessor implements IiifImageProcessor {

    private static final Logger log = LoggerFactory.getLogger(JnaIiifImageProcessor.class);
    private static final double EPS = 1e-10;

    private final VipsLibrary vips;
    private final IiifImageProcessor fallback;

    public JnaIiifImageProcessor(IiifImageProcessor fallback) {
        this(NativeLibraryDetector.getVipsLibrary(), fallback);
    }

    public JnaIiifImageProcessor(VipsLibrary vips, IiifImageProcessor fallback) {
        this.vips = vips;
    ...
    private Pointer applySize(Pointer image, IiifImageProcessor.Size size) {
        ...
        if (Math.abs(scaleW - scaleH) < EPS) {
            result = vips.vips_resize(image, out, scaleW, (Object) null);
        } else {
            // For now, let's just do proportional to stay simple, or use vips_resize twice? No.
            result = vips.vips_resize(image, out, scaleW, "vscale", scaleH, (Object) null);
        }
    ...
    private Pointer applyRotation(Pointer image, IiifImageProcessor.Rotation rotation) {
        ...
        if (deg != 0) {
            ...
            } else if (Math.abs(deg - 180.0) < EPS) {
                result = vips.vips_rot(current, out, 2, (Object) null);
            } else if (Math.abs(deg - 270.0) < EPS) {
    ...
    private Pointer applyQuality(Pointer image, IiifImageProcessor.Quality quality) {
        ...
        if (quality == IiifImageProcessor.Quality.GRAY) {
            ...
            Pointer gray = grayRef.getValue();
            // VIPS_OPERATION_RELATIONAL_MORE = 4
            if (vips.vips_relational_const(gray, out, 4, new double[]{128.0}, (Object) null) != 0) {
